...
\begin{align}
&\text{Recall that for a free relativistic particle the kinetic energy is}\quad T_\text{now}=m_0c^2(1-(1-\dot{x}_\text{now}^2/c^2)^{-1/2})\\
&\text{Rearranging yields}\quad \dot{x}_\text{now}=c\sqrt{1-m_0^2c^4/T_\text{now}^2}\\
&\text{Since}\quad T_\text{now}=T_\text{before}+s\dot{x}_\text{before}\Delta t\\
&\text{By substitution}\quad \dot{x}_\text{now}=c\sqrt{1-m_0^2c^4/(T_\text{before}+s\dot{x}_\text{before}\Delta t)^2}\\
&\text{Substituting to be in terms of }\dot{x}_\text{before} \text{ we have from the relativistic kinetic energy in the previous time step}\nonumber\\ 
&\dot{x}_\text{now}=c\sqrt{1-m_0^2c^4/(m_0c^2((1-\dot{x}_\text{before}^2/c^2)^{-1/2}-1)+s\dot{x}_\text{before}\Delta t)^2}
\end{align}
